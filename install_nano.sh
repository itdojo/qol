...
backup_file() {
    ...
    # own stderr is suppressed: a failure here already gets a clearer STOP
...
replace_file_contents() {
    ...
    # already prints a clearer STOP line of its own, and the raw tool message
...
#
# The caller keeps ownership of backups; this returns non-zero with a STOP line
...
write_nanorc() {
    ...
    # returns 1 with a STOP line of its own on failure, rather than leaving a
...
resolve_nano() {
    ...
    bin="$(command -v nano 2>/dev/null)" || {
        ...
        # first run. log_err's STOP badge reads as "something went wrong" right
  ...
  export VISUAL=nano"
    ...
    if [[ -n "$DRY_RUN" ]]; then
        ...
        while IFS= read -r rc; do
            # Indented to column 10, where the INFO above it starts its message.
            [[ -n "$rc" ]] && printf '         %s\n' "$rc"
        done <<EOF
$rcs
EOF
...
confirm() {
    ...
    #
    # Still not ask_confirm: that helper's own non-TTY rule would bypass the
    # QOL_NANO_FORCE_TTY seam this script's test suite drives every prompt
    # through. Borrow its shape — ASK badge, then a jade caret carrying the
    # default — and keep this function's semantics exactly as they were.
    local answer=""
    log_ask "$prompt" >&2
    printf '    %s❯%s %s[y/N]%s ' "$QOL_PASS" "$QOL_RESET" "$QOL_META" "$QOL_RESET" >&2
    read -r answer
    case "$answer" in
...
main() {
    ...
    banner "NANO SETUP" "syntax · defaults · cheat sheet"
    ...
    # summary and exit 0 — the WARN Skipped lines above had already scrolled
    ...
    if confirm "Write the qol block to $NANORC?"; then
        ...
        # own callers a meaningful non-zero return with a STOP line already
    ...
    if [[ -z "$nanorc_written" && -z "$editor_written" ]]; then
        ...
        if [[ -n "$SYNTAX_PACK_CHANGED" ]]; then
            log_warn "No config was written — every prompt was declined."
            log_warn "The community syntax pack was synced to $SYNTAX_DIR before those prompts,"
            log_warn "so that much did change; delete that directory if you don't want it."
        else
            log_warn "Nothing was changed — every prompt was declined."
        fi
        log_complete "NANO SETUP MADE NO CHANGES"
        log_next "Re-run and answer y, or pass --yes, to actually write the config."
        echo
        exit 0
    fi
    ...
    if [[ -n "$nanorc_written" ]]; then
        ...
        log_info "Config:      $NANORC"
        log_info "$syntax_line"
    else
        ...
        log_warn "$NANORC was not written (declined). nano itself is installed and usable."
    fi

    if [[ -n "$SET_EDITOR" && -z "$editor_written" ]]; then
        log_warn "EDITOR/VISUAL were not set (declined)."
    fi

    # Lowercase on purpose: the completion badge shouts the title, and this
    # line is the sentence the test suite reads to know the run finished.
    log_ok "nano is ready. Open a new file to see it."
    log_complete "NANO SETUP COMPLETE"
    log_next "Cheat sheet: $SCRIPT_DIR/docs/nano-cheatsheet.html"
    log_next "^S saves. ^X exits. ^Z suspends (fg to resume). M-U undoes, M-E redoes."
    echo
}
